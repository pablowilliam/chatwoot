...
Rake::Task['db:migrate'].enhance do
  if ActiveRecord::Base.connected? && ActiveRecord::Base.connection.table_exists?('installation_configs')
    puts 'Loading Installation config'
    ConfigLoader.new.process
  end
rescue ActiveRecord::NoDatabaseError
  puts 'Database does not exist yet. Skipping config load.'
rescue ActiveRecord::StatementInvalid => e
  puts "Error checking for table 'installation_configs': #{e.message}"
end
