  ...
  <form class="flex flex-col w-full" @submit.prevent="onSubmit">
    <div class="w-full">
      <label :class="{ error: v$.url.$error }">
        ...
        <input v-model.trim="url" type="text" name="url" :placeholder="webhookURLInputPlaceholder"
          @input="v$.url.$touch" />
      ...
      <div class="flex flex-col gap-2.5 mb-4">
        <div v-for="event in supportedWebhookEvents" :key="event" class="flex items-center">
          <input :id="event" v-model="subscriptions" type="checkbox" :value="event" name="subscriptions" class="mr-2" />
    ...
    <div class="flex flex-row justify-end w-full gap-2 px-0 py-2">
      ...
      <woot-button :disabled="v$.$invalid || isSubmitting" :is-loading="isSubmitting">
        {{ submitLabel }}
      </woot-button>
    </div>
  </form>
