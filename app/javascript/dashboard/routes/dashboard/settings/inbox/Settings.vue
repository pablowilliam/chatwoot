...
export default {
  ...
  data() {
    return {
      ...
      csatLockPeriodDays: 14, // Valor predeterminado del período de bloqueo de CSAT
    ...
    async updateInbox() {
      try {
        const payload = {
          ...
          csat_lock_period_days: this.csatLockPeriodDays,
  ...
  >
    ...
    <div v-if="selectedTabKey === 'inbox_settings'" class="mx-8">
      ...
      >
        ...
        <div v-if="csatSurveyEnabled" class="pb-4 w-3/4">
          <label for="csatLockPeriodDays">{{ $t('INBOX_MGMT.SETTINGS_POPUP.CSAT_LOCK_PERIOD') }}</label>
          <input
            type="number"
            id="csatLockPeriodDays"
            v-model.number="csatLockPeriodDays"
            min="1"
            class="input-field"
            :placeholder="$t('INBOX_MGMT.SETTINGS_POPUP.CSAT_LOCK_PERIOD_PLACEHOLDER')"
          />
          <p class="text-sm text-slate-600 dark:text-slate-400">
            {{ $t('INBOX_MGMT.SETTINGS_POPUP.CSAT_LOCK_PERIOD_HELP_TEXT') }}
          </p>
        </div>
