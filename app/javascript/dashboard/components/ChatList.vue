...
export default {
  ...
  computed: {
    ...
    conversationList() {
      ...
      if (!this.hasAppliedFiltersOrActiveFolders) {
        ...
        // Añadir validación adicional para prevenir acceso a conversaciones no asignadas
        if (this.hideUnassignedForAgents && this.activeAssigneeTab === 'unassigned') {
          return [];
        }
        ...
        // Para filtros aplicados, verificar que no incluyan conversaciones no asignadas
        if (this.hideUnassignedForAgents) {
          conversationList = [...this.chatLists].filter(chat => chat.assignee_id !== null);
        } else {
  ...
  methods: {
    ...
    onApplyFilter(payload) {
      // Añadir validación para agentes
      if (this.hideFiltersForAgents) {
        return;
      }
    ...
    updateAssigneeTab(selectedTab) {
      // Añadir validación para prevenir acceso a conversaciones no asignadas
      if (this.hideUnassignedForAgents && selectedTab === 'unassigned') {
        return;
      }
    ...
    initializeExistingFilterToModal() {
      // No permitir filtros de asignación para agentes si está oculto unassigned
      if (!this.hideUnassignedForAgents) {
